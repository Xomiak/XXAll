...
class Gallery extends CI_Controller
{
    ...
    public function index()
    {
        if (isset($_POST['delete']) && isset($_POST['list'])) {
            ...
            $count = count($list);
            for ($i = 0; $i < $count; $i++) {
                $image = $this->gallery->getFoto($list[$i]);
                if ($image) {
    ...
    public function categories()
    {
        ...
        $this->load->view('gallery_categories', $data);
    }
    ...
    function upload_category_foto($folder = false)
    { // Функция загрузки и обработки фото

        $document_root = '/upload/gallery/categories';
        if (!file_exists($document_root))
            mkdir($_SERVER['DOCUMENT_ROOT'] . $document_root, 0777);

        if ($folder) $document_root .= "/" . $folder;
        ...
        $config['upload_path'] = substr($document_root, 1);
        ...
        if (!$this->upload->do_upload()) {
            ...
            $gallery_mini_foto_width = $this->model_options->getOption("gallery_category_foto_width");
            if (!$gallery_mini_foto_width) $gallery_mini_foto_width = 200;

            $gallery_mini_foto_height = $this->model_options->getOption("gallery_category_foto_height");
    ...
    function upload_foto($article_name = '', $folder = '')
    {// Функция загрузки и обработки фото
...
//var_dump($folder);die();
        ...
        $data['left_sidebar'] = $this->load->view('common/left_sidebar.php',$data, true);
        $data['footer'] = $this->load->view('common/footer.php',$data, true);
        $this->load->view('gallery_categories_add', $data);
    }
    ...
    public function categoriesEdit($id)
    {
        ...
        $data['num'] = $this->gallery->getNewCategoryNum();
        $data['categories'] = $this->gallery->getHomeCategories();
        $this->load->view('gallery_categories_edit', $data);
    }
    ...
    public function addFoto()
    {
        ...
        $data['err'] = $err;
        //$data['categories'] = $this->gallery->getCategories();
        $this->load->view('gallery_foto_add', $data);
    }
    ...
    public function editFoto($id)
    {
        ...
        $data['err'] = $err;
        //$data['categories'] = $this->gallery->getCategories();
        $this->load->view('gallery_foto_edit', $data);
    }
    ...
    {
        $cat = $this->gallery->getFoto($id);
        if (($cat) && $cat['num'] < ($this->gallery->getNewImageNum() - 1)) {
            $num = $cat['num'] + 1;
            $oldcat = $this->gallery->getFotoByNum($num);
            $dbins = array('num' => $num);
            $this->db->where('id', $id)->limit(1)->update('gallery_images', $dbins);
            if ($oldcat) {
                $dbins = array('num' => ($num - 1));
                $this->db->where('id', $oldcat['id'])->limit(1)->update('gallery_images', $dbins);
            }
        }
        $url = '/admin/gallery/';
    ...
    public function options()
    {
        ...
        //$data['categories'] = $this->gallery->getCategories();
        $this->load->view('gallery_options', $data);
    }
    ...
    public function optionsEdit()
    {
        ...
        $data['main'] = $this->gallery->getMain();
        //$data['categories'] = $this->gallery->getCategories();
        $this->load->view('gallery_options_edit', $data);
    }
    ...
    public function zipImport()
    {
        ...
        $this->load->view('gallery_zip_import', $data);
    }
}