...
<head>
	...
	<link href="<?=ENGINE_PATH?>/favicon.ico" rel="shortcut icon" type="image/x-icon"/>
    ...
    elseif(strpos($_SERVER['REQUEST_URI'],'/page-') !== false)
        $canonical = request_uri(true);

	if($canonical)
		echo '
		<link rel="canonical" href="//'.$_SERVER['SERVER_NAME'].$canonical.'" />';

	if(isset($config['prev']))
        echo '
        <link rel="prev" href="//'.$_SERVER['SERVER_NAME'].$config['prev'].'" />';
	if(isset($config['next']))
        echo '
        <link rel="next" href="//'.$_SERVER['SERVER_NAME'].$config['next'].'" />';
	?>
