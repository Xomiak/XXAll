				...
				<div class = "content">
					<div class = "top_menu">
						...
						if ($modules) {
							...
							for ($i = 0; $i < $count; $i++) {
								...
								if ($this->session->userdata('quest_module_name') == $m['name'])
									echo ' selected';
								echo '>' . $m['title'] . '</option>';
							}
						}
					...
					<table width = "100%" cellpadding = "1" cellspacing = "1">
						...
						for ($i = 0; $i < count($opinions); $i++) {
							...
							<tr class = "list">
								<td>
									<a href = "/admin/interactive/edit_opinion/<?= $op['id'] ?>/" title = "Перейти к редактированию"><?= $op['id'] ?></a>
								</td>
								<td>
									<?= $op['user_name'] . '(' . $op['user_email'] . ')' ?>
								</td>
								<td>
									<?= (strlen($op['opinion']) > 100) ? substr($op['opinion'], 0, 100) . '...' : $op['opinion'] ?>
								</td>
								<td>
									<?= $op['date'] ?>
								</td>
								<td>
									<a href = "/admin/interactive/edit_opinion/<?= $op['id'] ?>/"><img src = "/img/edit.png" width = "16px" height = "16px" border = "0" title = "Редактировать"/></a>

									<a onclick = "return confirm('Удалить?')" href = "/admin/interactive/del_opinion/<?= $op['id'] ?>/"><img src = "/img/del.png" border = "0" alt = "Удалить" title = "Удалить"/></a>
