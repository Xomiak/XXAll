...
class Googlemaps {
	...
	var $apiKey						= 'AIzaSyDL-ughU0ynX4JHYIU7m32K3zVF_-0fKgQ'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	function get_address_from_lat_long($lat = 0, $lng = 0, $attempts = 0)
	{
		$error = '';
		...
		if ($this->geocodeCaching) { // if caching of geocode requests is activated
			...
			$CI->db->from("geocoding");
			$CI->db->where("latitude", $lat);
			$CI->db->where("longitude", $lng);
			$query = $CI->db->get();
		...
		$latlng = $lat.','.$lng;
		if ($this->https) { $data_location = 'https://'; }else{ $data_location = 'http://'; }
		$data_location .= "maps.google.com/maps/api/geocode/json?latlng=".urlencode($latlng)."&sensor=".$this->sensor;
		if ($this->region!="" && strlen($this->region)==2) { $data_location .= "&region=".$this->region; }
		$data = file_get_contents($data_location);
vd($data);
		$data = json_decode($data);
