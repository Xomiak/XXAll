...
class Model_admin extends CI_Model {
	...
	//Уведомления
	function getNotificationsCount($login = false, $status = -1){
		if($status != -1) $this->db->where('status', $status);

		if($login) {
			$this->db->where('login', $login);
			$this->db->or_where('login', NULL);
		} else $this->db->where('login', NULL);

		$this->db->from('admin_notifications');
		return $this->db->count_all_results();
	}

	function getNotifications($login = false, $status = -1){
		if($status != -1) $this->db->where('status', $status);
		...
		$this->db->order_by('date_unix','DESC');
		return $this->db->get('admin_notifications')->result_array();
	}

	function getNotification($id){
		$this->db->where('id', $id);
		$this->db->limit(1);
		$ret = $this->db->get('admin_notifications')->result_array();
