...
class Model_articles extends CI_Model
{
    ...
    function getArticlesByParam($param, $value, $per_page = -1, $from = -1, $active = -1, $order_by = "DESC", $not_in_categories = false)
    {
    ...
    function getArticlesByCategory($category_id = -1, $per_page = -1, $from = -1, $active = -1, $order_by = "DESC", $sort_by = 'num', $not_article_id = false)
    {
        if ($active != -1) {
            $this->db->where('active', $active);
        }
        ...
        if ($not_article_id)
            $this->db->where('id <>', $not_article_id);
    ...
    function getCountArticlesInSubCategories($parent_category_id, $active = -1)
    {
    ...
    function getArticlesByParentCategory($parent_category_id, $per_page = -1, $from = -1, $active = -1, $order_by = "DESC")
    {
        if ($active != -1)
            $this->db->where('active', $active);
        $this->db->where('parent_category_id', $parent_category_id);
    ...
    function getArticlesByFirstCategory($parent_category_id, $per_page = -1, $from = -1, $active = -1, $order_by = "DESC", $sort_by = 'num')
    {
        $this->db->where('first_category_id', $parent_category_id);
    ...
    function getArticleIdByUrlAndCategoryId($url, $category_id, $active = 1)
    {
        $this->db->select('id');
        $this->db->where('url', $url);
        if ($active != -1)
...
//			$this->db->where('active', $active);
    ...
    public function getCountArticlesByTag($tag, $per_page = -1, $from = -1, $active = -1)
    {
        if ($active != -1)
            $this->db->where('active', $active);
        if ($per_page != -1 && $from != -1)
